...
#Q6 : on voit sur le graphique précédent, que pour N=1e5, l'estimation est suffisament précise.
"""
N=100000
S0=2
nbr_rho = 40
...
#Q10 : pour N=500.000, on a une longueur d'intervalle de confiance normalisé d'environ de 0.007, on a donc 
#une erreur d'aproximation assez faible
...
def P2_controle(T,L,S0,sig,eps):
    ...
    X = np.exp(-r*T)*np.maximum(K-np.max(S,axis=1),0)
    Y = np.exp(-r*T)*np.maximum(K-S[:,0],0)

    E_Y = PE_i(T,r,sig,K,S0[0])
    opt = np.cov(X,Y,ddof=1)[0,1]/var_empi(Y)
    Z = X-opt*(Y-E_Y)
